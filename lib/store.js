...
/**
 ...
 store.push = (req, { name, data }) => {
    ...
    if (typeof name !== 'string') {
        throw error({ status : 500, code : 'INTERNAL_SERVER_ERROR', message : 'AMN: reference is not a string' });
    }
    if (!data || typeof data !== 'object') {
        throw error({ status : 500, code : 'INTERNAL_SERVER_ERROR', message : 'AMN: data is not an object' });
    }
...
store.pop = (req, { name, strict = true }) => {
    ...
    if (strict && !isExists) {
        throw error({ status : 404, code: 'NOT_FOUND', message: 'AMN: not such data to find' });
    }
